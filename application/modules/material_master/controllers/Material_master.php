...
class Material_master extends Admin_Controller
{
  ...
  public function data_side_material_master()
  {
    $this->Material_master_model->get_json_material_master();
  }
  ...
  public function toggle_status()
  {
    ...
    if ($this->db->trans_status() === FALSE) {
      $keterangan = "FAILED, change status Material: $id";
      $status = 0;
    } else {
      $status_text = ($new_status == 1) ? 'Active' : 'Inactive';
      $keterangan = "SUCCESS, change status Material ID: $id to $status_text";
      $status = 1;
    }
